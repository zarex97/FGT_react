...
export const NPImplementations = Object.values(ServantRegistry).reduce((acc, servantClass) => {
...
export const executeNP = (npRef, gameState, caster, targetX, targetY) => {
    ...
    if (isNPOnCooldown(npRef, gameState.currentTurn)) {
        ...
        return {
            success: false,
            message: `Noble Phantasm is on cooldown for ${npRef.onCooldownUntil - gameState.currentTurn} more turns`,
            error: 'ON_COOLDOWN',
            turnsRemaining: npRef.onCooldownUntil - gameState.currentTurn
        };
    }
...
export const getNPAffectedCells = (npImpl, caster, targetX, targetY, gridSize) => {
    if (!npImpl?.microActions?.[0]) {
        console.error('Invalid Noble Phantasm implementation or missing microActions');
        return new Set();
    }
