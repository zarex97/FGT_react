  ...
  static getAffectedCells(params) {
    ...
    switch (targetingType) {
      ...
      case TargetingType.AOE_AROUND_SELF:
        for (
          let x = Math.max(0, casterX - range);
          x <= Math.min(gridSize - 1, casterX + range);
          x++
        ) {
          for (
            let y = Math.max(0, casterY - range);
            y <= Math.min(gridSize - 1, casterY + range);
            y++
          ) {
            const distance = this.calculateChebyshevDistance(
              casterX,
            ...
            if (distance <= range) {
              if (
                !applyCornerRule ||
                !this.isCornerCell(casterX, casterY, x, y, range)
              ) {
      ...
      case TargetingType.AOE_FROM_POINT:
        if (targetX !== undefined && targetY !== undefined && dimensions) {
          ...
          const distanceToTarget = this.calculateChebyshevDistance(
            casterX,
            casterY,
            targetX,
            targetY
          ...
          if (distanceToTarget <= range) {
            ...
            for (let x = targetX - halfWidth; x <= targetX + halfWidth; x++) {
              for (
                let y = targetY - halfHeight;
                y <= targetY + halfHeight;
                y++
              ) {
                if (x >= 0 && x < gridSize && y >= 0 && y < gridSize) {
                  if (
                    !applyCornerRule ||
                    !this.isCornerCell(targetX, targetY, x, y, range)
                  ) {
                    affectedCells.add(`${x},${y}`);
                  }
                }
          ...
          if (distanceToTarget <= range) {
            ...
            for (let x = targetX - halfWidth; x <= targetX + halfWidth; x++) {
              for (
                ...
                if (x >= 0 && x < gridSize && y >= 0 && y < gridSize) {
                  const distanceToCaster = this.calculateChebyshevDistance(
                    casterX,
                    casterY,
                    x,
                    y
                  ...
                  if (distanceToCaster <= range) {
                    // Key difference: check distance to caster
