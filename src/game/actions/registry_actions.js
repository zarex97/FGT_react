...
export const ActionImplementations = Object.values(ServantRegistry).reduce(
  (acc, servantClass) => {
    Object.values(servantClass).forEach((servantInfo) => {
      // Get character name for namespacing
      const characterName = servantInfo.template?.name;

      console.log("Processing servant for Actions:", {
        name: characterName,
      ...
      if (servantActions && characterName) {
        // Process common actions with namespacing
        // Even "common" actions might have character-specific implementations
        if (servantActions.common) {
          if (!acc.common) acc.common = {};
          Object.entries(servantActions.common).forEach(
            ([actionId, action]) => {
              const uniqueKey = `${characterName}_${actionId}`;
              console.log(
                "Registering common action with unique key:",
                uniqueKey
              );
              acc.common[uniqueKey] = action;
            }
          );
        }

        // Process unique actions with namespacing
        if (servantActions.unique) {
          if (!acc.unique) acc.unique = {};
          Object.entries(servantActions.unique).forEach(
            ([actionId, action]) => {
              const uniqueKey = `${characterName}_${actionId}`;
              console.log(
                "Registering unique action with unique key:",
                uniqueKey
              );
              acc.unique[uniqueKey] = action;
            }
          );
        }
      }
...
export const getActionAffectedCells = (
  actionImpl,
  caster,
  targetX,
  targetY,
